...
class RBM:
    ...
    def prob_v_given_h(self, h):
        ...
        prob_v = 1 / (1 + np.exp(-2*eff_mag)) 
        return prob_v

    def get_newV(self, h):
        prob_v = self.prob_v_given_h(h)
        newV = np.where(np.random.rand(self.num_visible) < prob_v, 1, -1) 
        return newV

    def prob_h_given_v(self, v):
        eff_mag = np.dot(v, self.weights) + self.hidden_biases
        prob_h = 1 / (1 + np.exp(-2*eff_mag)) 
        return prob_h

    def get_newH(self, v):
        prob_h = self.prob_h_given_v(v)
        newH = np.where(np.random.rand(self.num_hidden) < prob_h, 1, -1) 
        return newH
    ...
    def gibbs_sampling(self, k, num_samples):
        ...
        for _ in range(num_samples):
            v = np.random.choice([-1, 1], self.num_visible)
            h = np.random.choice([-1, 1], self.num_hidden)
            for _ in range(k):
                v = self.get_newV(h)
                h = self.get_newH(v)
            vf.append(v)
